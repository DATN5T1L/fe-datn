...
const EditRouter: React.FC<EditRouterProps> = (props) => {
        ...
        if (token) {
            fetch(`/api/allRouterAdmin/${id}`, {
                ...
                .then(data => {
                    ...
                    if (data) {
                        formik.setValues({
                            name_route: data.route.name_route || '',
                            discription_route: data.route.discription_route || '',
                            img_route: null
                        })
                        if (data.route.img_route) {
                            setPreviewImage(data.route.img_route)
                        }
                    }
                })
        }
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            if (token && id) {
                if (confirm('Bạn có muốn sửa lộ trình này không?')) {
                    const formData = new FormData();
                    if (values.img_route) {
                        formData.append("img_route", values.img_route);
                    }
                    try {
                        const res = await fetch(`/api/allRouterAdmin/${id}`, {
                            method: 'PUT',
                            headers: {
                                Authorization: `Bearer ${token}`,
                            ...
                            body: JSON.stringify({
                                name_route: values.name_route,
                                discription_route: values.discription_route
                            }),
                        });
                        const resImgresImg = await fetch(`/api/updateImgRoute/${id}`, {
                            method: 'POST',
                            headers: {
                                Authorization: `Bearer ${token}`,
                            },
                        ...
                        const dataImg = await resImgresImg.json();
                        if (data.status === 'success') {
                            alert('Sửa lộ trình thành công!!!')
                            router.replace('/giangvien/router')
                        } else {
                            alert('Sửa lộ trình thất bại')
                        }
                        console.log(data);
                        console.log(dataImg);
                    } catch (error) {
                ...
                alert("Vui lòng đăng nhập trước khi sửa lộ trình.");
            }
        }
    })
                        ...
                        <Form className={styles.form__container} onSubmit={formik.handleSubmit}>
                            ...
                            <Row className={styles.form__container__bottom}>
                                ...
                                <section className={styles.btn__group}>
                                    <Button>Hủy</Button>
                                    <Button type="submit">Sửa lộ trình</Button>
                                </section>
                            </Row>
                        </Form>
                    </Col>
